...
int main()
{
    string filename = string(ROOT_DIR) + "/" + string("test.jpg");
    ...
    Mat gray(height, width, CV_8UC1, input_data.get());

    shared_ptr<float> output_data(new float[size]{0, });
    fastCornerDetection(input_data.get(), output_data.get(), width, height, 200, 300);
    ...
    imwrite(string(RES_DIR)+"/"+string("result.jpg"), img);
    cout << "Finished" << endl;
}
