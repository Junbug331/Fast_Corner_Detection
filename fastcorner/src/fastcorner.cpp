...
void fastCornerDetection(uchar *input, float *output, int width, int height, int T1, int T2, int window_size, float scaleX, float scaleY)
{
    /**
    ...
    auto[low_res, low_width, low_height] = reduceImage(input, width, height, scaleX, scaleY);
    std::cout << "low: " << low_width << ", " << low_height << std::endl;
    std::cout << "input: " << width << ", " << height << std::endl;

    int padding = (window_size >> 1);
    ...
    {
        uchar* preRow = &low_res[(r-padding) * low_width];
        uchar* currRow = &low_res[r * low_width];
        uchar* nextRow = &low_res[(r+padding) * low_width];
        for (int c=padding; c<low_width-padding; ++c)
        {
            // Simple cornerness measure on low resolution image
            // vertical
            IC = currRow[c];
            ...
            if (C_sim > T1)
            {
                ...
                IB1 = input[(y-padding)*width + x];
                IB2 = input[(y+padding)*width + x];
                rA = (IA1 - IC)*(IA1 - IC) + (IA2 - IC)*(IA2 - IC);
                rB = (IB1 - IC)*(IB1 - IC) + (IB2 - IC)*(IB2 - IC);
                C_sim = std::min(rA, rB);
